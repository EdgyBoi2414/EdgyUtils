...
/**
 * Class for maintaining and handling connection to Redis server. It automatically fires
 * Bungee/Spigot events corresponding to current server type.
 * <p>
...
public class RedisManager {
   ...
   */
  private final IRedisPlugin plugin;
  ...
  /**
   * Constructor method for creating {@link RedisManager} instance. Constructor does not subscribe
   * to the channels, it just stores the provided data for the future.
   *
   * @param plugin             Origin plugin which tries to obtain the instance
   * @param serverIdentifier   Identifier of the server (e.g. 'Bungee01'). Shall be unique to
   *                           prevent bugs
   * @param redisConfiguration {@link RedisConfiguration} object with Redis server credentials
   */
  public RedisManager(IRedisPlugin plugin, String serverIdentifier,
   ...
   *
   * @param serverIdentifier   New server identifier (if null, already using server id will be
   *                           used)
   * @param redisConfiguration New RedisConfiguration (if null, already using configuration will be
   *                           used)
   */
  public void reload(String serverIdentifier, RedisConfiguration redisConfiguration) {
    this.close();
    this.closing = false;
   ...
   * <p>
   * Note! It does not return false if subscription itself was unsuccessful as the calls are
   * asynchronous.
   *
   * @param channels Default list channels to listen on (case-sensitive), can be empty and provided
   *                 afterwards
  ...
  public boolean setup(String... channels) {
    ...
    if (channels != null && channels.length > 0) {
      this.channels.addAll(new HashSet<>(Arrays.asList(channels)));
  ...
  public void unsubscribe(String... channels) {
    ...
    try {
      ...
      this.plugin.logger()
          .info("Successfully unsubscribed channels: " + Arrays.toString(channels) + "!");
    } catch (Exception ex) {
      this.plugin.logger().warning(
          "An error occurred while unsubscribing channels: " + Arrays.toString(channels) + "!");
      return;
    }

    this.channels.removeAll(new HashSet<>(Arrays.asList(channels)));
  }
  ...
  /**
   * Publishes the object to the provided channel. Also handles server identification. DO NOT USE
   * this to publish simple {@link String} message.
   *
   * @param targetChannel   Channel to be published into (case-sensitive)
   * @param objectToPublish Object to be published
   * @return Returns 'false' if the message cannot be converted to JSON or in closing state. Returns
   * 'true' if the process was successful
   * @see #publishMessage(String, String)
   */
  public boolean publishObject(String targetChannel, Type type, Object objectToPublish) {
    MessageTransferObject messageTransferObject = MessageTransferObject.wrap(this.serverIdentifier,
        type, objectToPublish, System.currentTimeMillis());
  ...
  /**
   * Publishes the message to the provided channel. Also handles server identification. This method
   * is not recommended for publishing serialized objects. To publish objects:
   *
   * @param targetChannel    Channel to be published into (case-sensitive)
  ...
  public boolean publishMessage(String targetChannel, String messageToPublish) {
    MessageTransferObject messageTransferObject = new MessageTransferObject(this.serverIdentifier,
        messageToPublish, System.currentTimeMillis());
    return this.executePublish(targetChannel, messageTransferObject);
  }
   ...
   */
  private boolean executePublish(String targetChannel,
    ...
    DebugLogger.info("Publishing message to channel '" + targetChannel + "': " + messageJson);
  ...
  private class Subscription extends JedisPubSub implements Runnable {
    ...
    public void run() {
      ...
      while (!RedisManager.this.closing && !Thread.interrupted()
          && !RedisManager.this.jedisPool.isClosed()) {
        try (Jedis jedis = RedisManager.this.jedisPool.getResource()) {
          ...
          try {
            ...
            RedisManager.this.plugin.logger()
                .info("Successfully subscribed channels: " + Arrays.toString(channels) + "!");
          } catch (Exception e) {
          ...
          RedisManager.this.plugin.logger()
              .warning("Redis pubsub connection dropped, trying to re-open the connection!");
          try {
            unsubscribe();
    ...
    public void onMessage(String channel, String message) {
      ...
      if (messageTransferObject == null) {
        RedisManager.this.plugin.logger().warning(
            "Cannot retrieve message object sent to channel '" + channel + "'! Message: '" + message
                + "'");
        return;
      }
   ...
   * @param redisConfiguration {@link RedisConfiguration} object with Redis server credentials
   */
  public static void init(IRedisPlugin plugin, String serverIdentifier,
      RedisConfiguration redisConfiguration) {
  ...
  /**
   * Gets the main instance of {@link RedisManager} object. This is the only recommended approach to
